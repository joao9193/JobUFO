...
class HomeComponent extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.getMovies.fetchingSuccess) {
      if (this.state.insearchMode && this.state.inserachFirst) {
        //In search mode
        ...
        if (this.state.insearchModeRelease) {
          //If true search box is just cleared , so clear the current array and push new results
          this.setState(
            {
              movieList: [],
              page: nextProps.getMovies.payload.page,
      ...
      <Container>
        <Container className="searchBar">
          <Row>
            <Col md="8" sm="12" xs="12" className="pd520 ">
              <Label for="exampleSelectMulti" className="homePageHeading">
                Popular Movies
              </Label>
            </Col>
            <Col md="4" sm="12" xs="12" className="pd520">
              <InputGroup>
                ...
                <InputGroupAddon addonType="append">
                  <InputGroupText className={this.state.searchName.length > 2 ? 'input-group-text-override' : null}>
                    {' '}
                    <FaSearchPlus />{' '}
                  </InputGroupText>
                </InputGroupAddon>
        ...
        <Row className="homeMovieCards">
          {this.state.movieList &&
            this.state.movieList.map(function(val, index) {
              return (
                <Col key={index} md="3" xs="12" sm="6" onClick={() => this.goToDetails(val.id)}>
                  <Card className="movieTile">
                    ...
                    <CardBody>
                      ...
                      <div>
                        {val.genre_ids &&
                          val.genre_ids.map(function(genresfilm, index) {
                            return (
                              <span key={'genercontainer_' + genresfilm + '_' + index}>
                                {this.state.genres &&
                                  this.state.genres.map(function(geners, index) {
                                    if (geners.id == genresfilm) {
                                      return (
                                        <span key={'gener_' + geners.id + '_' + index}>
                                          <Badge className="customBadge" color="primary">
                                            {geners.name}
                                          </Badge>
                                        </span>
                                      );
