      ...
      <Container>
        ...
        <Row className="myFavouritesCards">
          {this.state.datas && this.state.datas.length > 0 ? (
            this.state.datas.map(function(val, index) {
              return (
                <Col key={index} md="3" xs="12" sm="6">
                  <Card className="movieTile">
                    <div onClick={() => this.removeFromFavourites(index)}>
                      <i className="fa removeBtn fas fa-times-circle" />
                    </div>
                    ...
                    <CardBody onClick={() => this.goToDetails(val.id)}>
                      ...
                      <div>
                        {val.genres &&
                          val.genres.map(function(vals, index) {
                            return (
